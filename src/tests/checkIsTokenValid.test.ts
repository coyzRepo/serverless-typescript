...
test('token should be valid', () => {
    ...
    UserService.jwtVerify = mockVerify;
    UserService.dateFnsDifferenceInHours = mockDifferenceInHours;
    expect(UserService.checkIsTokenValid(token, privateKey)).toBe(true);
});
...
test('token should not be valid', () => {
    ...
    UserService.jwtVerify = mockVerify;
    UserService.dateFnsDifferenceInHours = mockDifferenceInHours;
    expect(UserService.checkIsTokenValid(token, privateKey)).toBe(false);
});